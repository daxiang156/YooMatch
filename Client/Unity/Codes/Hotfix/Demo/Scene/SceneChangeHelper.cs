namespace ET
{
    public static class SceneChangeHelper
    {
        ...
        public static async ETTask SceneChangeTo(Scene zoneScene, string sceneName, long sceneInstanceId)
        {
            ...
            Log.Console("SceneChangeTo sceneName " + sceneName);
            if (sceneName == "scene_City")
            {
                EventDispatcher.PostEvent(EventName.AppsFlyEvent, null, AppsEventName.scene_City_Load);
                EventDispatcher.PostEvent(EventName.FireBEvent, null, FireBEventName.scene_City_Load);
            }
            ...
            //创建战斗场景
            if(sceneInstanceId == 6)
                Game.EventSystem.Publish(new EventType.BattleSceneFinish(){battleScene = currentScene});
