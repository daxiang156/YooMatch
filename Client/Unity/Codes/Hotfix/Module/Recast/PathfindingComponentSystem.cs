...
namespace ET
{
    public static class PathfindingComponentSystem
    {
        ...
        public static void Find(this PathfindingComponent self, Vector3 start, Vector3 target, List<Vector3> result)
        {
            ...
            int n = 3;//Recast.RecastFind(self.NavMesh, PathfindingComponent.extents, self.StartPos, self.EndPos, self.Result);
            for (int i = 0; i < n; ++i)
            {
                int index = i * 3;
        ...
        public static Vector3 RecastFindNearestPoint(this PathfindingComponent self, Vector3 pos)
        {
            ...
            int ret = 3;//Recast.RecastFindNearestPoint(self.NavMesh, PathfindingComponent.extents, self.StartPos, self.EndPos);
            if (ret == 0)
            {
