    ...
    public sealed class Session: Entity, IAwake<AService>
    {
        ...
        public override void Dispose()
        {
            ...
            foreach (RpcInfo responseCallback in this.requestCallbacks.Values.ToArray())
            {
                ...
                Log.Info($"session dispose: {id} {this.RemoteAddress}");
            }
        ...
        {
            //if (!GameDataMgr.isLoginFinish)
            // {
            //     if (request is IActorLocationRequest)
            //     {
            //         Log.Error("loging not finish fobbid c2m request :" + request.GetType());
