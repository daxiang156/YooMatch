...
[Header(SSS)][Toggle(_SKIN_SSS)] _skinSSS("skin SSS", Float) = 0
[NoScaleOffset]_SssLut("Sss Lut", 2D) = "white" {}
_SSSIntensity("SSSIntensity", Float) = 1

[Header(Emission)]_MaskMap_02("MaskMap_02", 2D) = "black" {}
[HDR]_GlowColor("GlowColor MaskMap_02(R)", Color) = (0,0,0,0)

//[Header(ANIS)][Toggle(_ANIS_SPECULAR)] _anisSpecular("ANIS SPECULAR", Float) = 0
//_AnisoControl("AnisoControl", Vector) = (0.25,0.8,0.1,1)
[Header(DecalsTex)]
_DecalsTex("DecalsTex", 2D) = "black" {}

[Header(RIM LIGHT)]
[HDR]_NiGuangClo("NiGuangClo", Color) = (1,1,1,0)
_NiGuang("NiGuang", Range(0 , 1)) = 0.5
_SideLightColor("SideLightColor", Color) = (1,1,1,1)

[Toggle(_TONEMAPOFF)] _TONEMAPOFF("TONEMAPOFF", Float) = 0
...
[HideInInspector] _DstBlend("__dst", Float) = 0.0
    ...
    SubShader
    {
        ...
        TEXTURE2D(_MaskMap_02);        SAMPLER(sampler_MaskMap_02);
        ...
        Pass
        {
            ...
            #pragma shader_feature_local_fragment _TONEMAPOFF
            ...
            #define DISSLOVE 1
            #define NEEDHIT           1
            #define SKIN_SSS_ENABLE 0
#else
            #if _SKIN_SSS
                #define SKIN_SSS_ENABLE 1
            #else
                 #define SKIN_SSS_ENABLE 0
            #endif
...
// [Lazarov 2013, "Getting More Physical in Call of Duty: Black Ops II"]
        ...
        half4 frag(Varyings input) : SV_Target
        {
            ...
            indirectSpecular.xyz = indirectSpecular.xyz * EnvBRDFApprox1(brdfSpecular, perceptualRoughness, NoV);//EnvironmentBRDFSpecular(roughness2, brdfSpecular, grazingTerm, fresnelTerm);
...
#endif
            half2 mask2 = SAMPLE_TEXTURE2D(_MaskMap_02, sampler_MaskMap_02, input.texcoord.xy).xy;

            finalColor.xyz += _GlowColor.xyz * mask2.xxx * albedo.xyz;
#if DISSLOVE
            finalColor.xyz += Emisson184.xyz;
#endif

            NoV = dot(V, vertexNormalDir);
            half rim = (1.0 - max(NoV, 0.0));
            half ramp = smoothstep((1.0 - _NiGuang), 1.0, rim );
            half NoL = dot(_MainLightPosition.xyz, vertexNormalDir);
            half fresnel = _SideLightScale * (rim * rim);
            finalColor.xyz += saturate(((_NiGuangClo.xyz * (ramp * (NoL + 0.1) * mask2.y) ) ));
        ...
        Pass
        {
            ...
            half4 LowFragment(Varyings i) : SV_Target
            {
                return  SAMPLE_TEXTURE2D(_MainTex, sampler_MainTex, i.texcoord.xy);
            }
            ENDHLSL
        }
